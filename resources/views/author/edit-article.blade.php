        ...
        <section class="section">
            <div class="card">
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        @if ($article->status == 'draft')
                            <form method="POST" action="{{ url('send-for-approval') }}">
                                ...
                                <button class="btn btn-primary rounded-pill btn-sm ms-2" type="submit">Send For
                                    Approval</button>
                            </form>
                        @endif
                    </div>
                    ...
                    @endif
                    <form class="row g-3" id="article" enctype="multipart/form-data" method="PUT"
                        action="{{ url("yn-author/articles/{$article->id}") }}">
                        @csrf
                        ...
                        <div class="col-md-6 col-12">
                            ...
                            <input type="date" class="form-control" id="date" name="created_at"
                                value="<?= date_format(date_create($article->created_at), 'Y-m-d') ?>">
                        </div>
                        <div class="col-12 col-md-6">
                            <label for="#category" class="form-label">Category</label>
                            <select class="form-select" id="category" name="category" required="">
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}" <?php echo $article->category == $category->id ? 'selected' : ''; ?>>
                                        {{ $category->category }}
                                    </option>
                                @endforeach
                            </select>
